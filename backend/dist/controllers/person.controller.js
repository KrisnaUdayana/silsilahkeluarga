...
exports.searchPersons = exports.getSiblings = exports.getChildren = exports.getFamilyTree = exports.deletePerson = exports.updatePerson = exports.createPerson = exports.getPersonById = exports.getAllPersons = void 0;
const prisma_1 = __importDefault(require("../utils/prisma"));
const http_1 = require("../utils/http");
const parsePersonDates = (birthDate, deathDate) => {
    const parsedBirthDate = parseOptionalDateField(birthDate, 'Tanggal lahir');
    if (parsedBirthDate.ok === false)
        return { ok: false, error: parsedBirthDate.error };
    const parsedDeathDate = parseOptionalDateField(deathDate, 'Tanggal wafat');
    if (parsedDeathDate.ok === false)
        return { ok: false, error: parsedDeathDate.error };
    if (parsedBirthDate.value && parsedDeathDate.value && parsedDeathDate.value < parsedBirthDate.value) {
        return { ok: false, error: 'Tanggal wafat tidak boleh sebelum tanggal lahir.' };
    }
    return {
        ok: true,
...
const parseOptionalDateField = (value, fieldName) => {
    if (value === undefined) {
        ...
        if (person.gender === 'MALE') {
            for (const child of person.childrenAsFather) {
                if (child.motherId && !inferredSpousesMap.has(child.motherId)) {
                    ...
                    if (!formalSpouses.some((s) => s.id === child.motherId)) {
                        const mother = await prisma_1.default.person.findUnique({
            ...
            for (const child of person.childrenAsMother) {
                if (child.fatherId && !inferredSpousesMap.has(child.fatherId)) {
                    // Check if not already in formal spouses
                    if (!formalSpouses.some((s) => s.id === child.fatherId)) {
                        const father = await prisma_1.default.person.findUnique({
...
const createPerson = async (req, res) => {
    try {
        ...
        if (!fullName || !gender) {
            return (0, http_1.validationError)(res, 'Nama lengkap dan jenis kelamin diperlukan.');
        }
        if (gender !== 'MALE' && gender !== 'FEMALE') {
            return (0, http_1.validationError)(res, 'Jenis kelamin harus MALE atau FEMALE.');
        }
        const parsedDates = parsePersonDates(birthDate, deathDate);
        if (parsedDates.ok === false) {
            return (0, http_1.validationError)(res, parsedDates.error);
        }
        // Validate parent IDs if provided
        if (fatherId) {
            const father = await prisma_1.default.person.findUnique({ where: { id: fatherId } });
            if (!father || father.gender !== 'MALE') {
                return (0, http_1.validationError)(res, 'Ayah tidak ditemukan atau bukan laki-laki.');
            }
        }
        if (motherId) {
            const mother = await prisma_1.default.person.findUnique({ where: { id: motherId } });
            if (!mother || mother.gender !== 'FEMALE') {
                return (0, http_1.validationError)(res, 'Ibu tidak ditemukan atau bukan perempuan.');
            }
        }
...
const updatePerson = async (req, res) => {
    try {
        const { id } = req.params;
        const { fullName, nickname, gender, birthDate, deathDate, birthPlace, occupation, biography, fatherId, motherId, profilePhoto } = req.body;
        const existing = await prisma_1.default.person.findUnique({ where: { id: String(id) } });
        if (!existing) {
            return res.status(404).json({ error: 'Anggota keluarga tidak ditemukan.' });
        }
        ...
        if (fatherId) {
            ...
            if (fatherId === id) {
                return (0, http_1.validationError)(res, 'Tidak bisa menjadi ayah dari diri sendiri.');
            }
        }
        if (motherId) {
            const mother = await prisma_1.default.person.findUnique({ where: { id: motherId } });
            ...
            if (motherId === id) {
                return (0, http_1.validationError)(res, 'Tidak bisa menjadi ibu dari diri sendiri.');
            }
        }
        const updateData = {};
        ...
        if (birthDate !== undefined)
            updateData.birthDate = parsedDates.birthDate;
        if (deathDate !== undefined)
            updateData.deathDate = parsedDates.deathDate;
        ...
        const person = await prisma_1.default.person.update({
            where: { id: String(id) },
            data: updateData,
            include: {
                father: { select: { id: true, fullName: true } },
...
const deletePerson = async (req, res) => {
    try {
        ...
        const existing = await prisma_1.default.person.findUnique({
            where: { id: personId },
            include: {
        ...
        if (existing._count.childrenAsFather > 0 || existing._count.childrenAsMother > 0) {
            return (0, http_1.validationError)(res, 'Tidak bisa menghapus anggota yang memiliki anak. Hapus hubungan anak terlebih dahulu.');
        }
        const linkedUser = await prisma_1.default.user.findUnique({ where: { personId } });
        if (linkedUser) {
            return (0, http_1.validationError)(res, 'Tidak bisa menghapus anggota yang masih terhubung dengan akun user.');
        }
        await prisma_1.default.$transaction([
            prisma_1.default.marriage.deleteMany({
                where: { OR: [{ husbandId: personId }, { wifeId: personId }] }
            }),
            prisma_1.default.person.delete({ where: { id: personId } })
        ]);
    ...
    try {
        ...
        const nodes = persons.map((person) => {
            ...
            const inferredSpouses = person.gender === 'MALE'
                ? person.childrenAsFather.map((c) => c.motherId).filter(Boolean)
                : person.childrenAsMother.map((c) => c.fatherId).filter(Boolean);
            // Unique spouse IDs
...
const getSiblings = async (req, res) => {
    try {
        ...
        const siblings = await prisma_1.default.person.findMany({
            where: {
                id: { not: personId },
                OR: orConditions
            },
...
const searchPersons = async (req, res) => {
    try {
        ...
        if (!Number.isInteger(take) || take < 1 || take > 50) {
            return (0, http_1.validationError)(res, 'Limit pencarian harus angka 1 sampai 50.');
        }
        const persons = await prisma_1.default.person.findMany({
            where: {
                OR: [
